...
LION具备以下设置项：
1. **识别间隔**：即程序执行OCR操作的频率，取值范围为0.1秒至10秒。
2. **识别目标**：用于指定进行OCR操作的屏幕区域。可供选择的选项包括：当前控件、当前窗口、导航对象以及全屏。
...
4. **文本相似度阈值**：用于控制新识别文本与上一轮结果相差多少时才朗读。滑块每次以10%的步长调整。
5. **正则表达式过滤器**：用于删除识别结果中匹配的内容，每行填写一个表达式。
